...
Deno.test("getBlockAudio rejects a missing blockId", async () => {
  const err = await assertRejects(
    () =>
      PollyService.getBlockAudio({
...
Deno.test("getBlockAudio rejects a missing characterId", async () => {
  const err = await assertRejects(
    () =>
      PollyService.getBlockAudio({
...
Deno.test("getBlockAudio rejects a blank characterId", async () => {
  const err = await assertRejects(
    () =>
      PollyService.getBlockAudio({
        blockId: "11111111-1111-1111-1111-111111111111",
